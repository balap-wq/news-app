...
// :white_check_mark: Mock functions
const mockFindTopHeadlines = jest.fn();
const mockCountArticles = jest.fn();

// :white_check_mark: Mock service BEFORE importing app
...
// :white_check_mark: Import app AFTER mock
...
describe('GET /api/headlines Integration Tests', () => {
  ...
  it('should return 200 with paginated data', async () => {
    ...
    const response = await request(app).get('/api/headlines?page=1&limit=5');
  ...
  it('should return 400 for invalid category', async () => {
    const response = await request(app).get('/api/headlines?category=invalid');
  ...
  it('should return 400 when page is 0', async () => {
    const response = await request(app).get('/api/headlines?page=0');
